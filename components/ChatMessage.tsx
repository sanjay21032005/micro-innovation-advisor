...
import { User, Sparkles, MessageCircle, ThumbsUp, ThumbsDown, Volume2, StopCircle, Bookmark, Check } from 'lucide-react';

interface ChatMessageProps {
  message: ChatMessageType;
  onRefine: (text: string) => void;
  onFollowUp: (text: string) => void;
  onSave?: (message: ChatMessageType) => Promise<void>;
}
...
const ChatMessage: React.FC<ChatMessageProps> = ({ message, onRefine, onFollowUp, onSave }) => {
  ...
  return (
    <div className={`w-full py-8 ${isUser ? 'bg-transparent' : 'bg-slate-50/50'}`}>
      <div className="max-w-3xl mx-auto px-4 flex gap-4 sm:gap-6">
        {/* Avatar */}
        <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center shadow-sm ${isUser ? 'bg-white border border-slate-200' : 'bg-indigo-600 border border-indigo-600'
          }`}>
          {isUser ? (
              ...
              <div className="flex items-center gap-1">
                {onSave && (
                  <button
                    ...
                    className={`p-1 rounded-md transition-colors ${isSaved ? 'text-green-600 bg-green-50' : 'text-slate-400 hover:text-slate-600 hover:bg-slate-100'}`}
