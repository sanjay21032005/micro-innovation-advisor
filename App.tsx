...
const App: React.FC = () => {
  ...
  // If viewing terms, show it regardless of login state
  if (currentView === 'terms') {
    return <TermsView onBack={() => setCurrentView(user ? 'chat' : 'chat')} />;
  }

  if (!user) {
    return (
      <AuthScreen 
        onLogin={(name, email, avatar) => setUser({name, email, avatar})} 
